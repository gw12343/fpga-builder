...
#define NOTIFY_PADDING_X 60.f // Bottom-left X padding
#define NOTIFY_PADDING_Y 60.f // Bottom-left Y padding
#define NOTIFY_PADDING_MESSAGE_Y 15.f // Padding Y between each message
...
#define NOTIFY_OPACITY 0.65f // 0-1 Toast opacity
...
#define NOTIFY_RENDER_OUTSIDE_MAIN_WINDOW                                                                              \
    false // If true, the notifications will be rendered in the corner of the monitor, otherwise in the corner of the
          // main window
...
namespace ImGui {
    ...
    inline void RenderNotifications() {
        ...
        for (size_t i = 0; i < notifications.size(); ++i) {
...
#else
            // Set notification window position to bottom right corner of the main window, considering the main window
            // size and location in relation to the display
            ...
            {
                PushTextWrapPos(mainWindowSize.x / 3.f); // We want to support multi-line text, this will wrap the text
                                                         // after 1/3 of the screen width
